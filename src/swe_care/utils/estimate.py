...
from swe_care.utils.classify_code_review_tasks import (
    classify_problem_domain,
    estimate_review_effort,
    estimate_review_difficulty,
)
from swe_care.utils.llm_models import init_llm_client


def estimate_problem_domains(
    pr_data: dict[str, Any], problem_statement: str
) -> list[str]:
...
def estimate_difficulty(
    pr_data: dict[str, Any], commit_message: str, patch: str
) -> str:
...
def classify_review_effort(
    ...
    if "nodes" in labels:
        for node in labels["nodes"]:
            if "Review effort" in node.get("name"):
                return int(
                    node.get("name").split("Review effort")[-1].strip().split("/")[0]
                )
    client = init_llm_client(
        model="gemini-2.5-pro-preview-06-05",
        model_provider="openai",
    ...
    return estimate_review_effort(
        client, pr_data.get("title", ""), pr_data.get("body", ""), commit_message, patch
    )
